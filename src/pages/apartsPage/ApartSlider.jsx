    ...
    {apartPictures[0] &&
      <>
        <div className='slider__house'>
          ...
          <img className="slider__house-front" src={`https://gelenrest.ru.ru${apartPictures[currentIndex].url}`} />
        ...
        <div className='slider__house-photos'>
          {apartPictures.map((item, index) =>
            currentIndex !== index ? (
              <img key={index} id={index} className='house_photo' src={`https://gelenrest.ru.ru${item.url}`} alt={`Image ${index}`} />
            ) : null
          )}
        </div>
      </>
