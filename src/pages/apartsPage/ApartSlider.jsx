...
export default function ApartSlider({ apartPictures }) {
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? apartPictures.length - 1 : prev - 1
    );
  };

  const nextSlide = () => {
    setCurrentIndex((prev) =>
      prev === apartPictures.length - 1 ? 0 : prev + 1
    );
  };
  ...
  const handleClickOutside = (e) => {
    if (
      isActive &&
      !e.target.closest(
        ".slider__house-front, .house__slider-prev, .house__slider-next"
      )
    ) {
      setIsActive(false);
    }
        ...
        <>
          ...
          >
            ...
            <img
              ...
              onClick={toggleMenu}
              src={`https://api.gelenrest.ru${apartPictures[currentIndex].url}`}
            />
            ...
            {apartPictures.map((item, index) =>
              currentIndex !== index ? (
                <img
                  ...
                  src={`https://api.gelenrest.ru${item.url}`}
                  alt={`Image ${index}`}
                />
              ) : null
            )}
